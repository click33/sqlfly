...
public class SqlFly extends SqlFlyBase {
	 ...
	 *  <p>这意味着Fly不会自动在处理完毕结果集后将其关闭，
	 * <p> 因此你必须在处理完结果集后执行rs.getStatement().close()释放资源; 
	 * <p>并且在调用此方法之前必须开启事务
	 * @param sql sql
	 * @param args args
	 * @return 返回值 
	 ...
	 * <p> long id = FlyFactory.getFly().getModel(long.class, "SELECT @@identity");
	 * @param cs 要映射的类型 
	 * @param sql sql
	 * @param args args
	 * @return v
	 ...
	 * <h1>支持基本数据类型，如：Integer、Double等</h1> 参数：sql语句，映射类型，参数列表<p>
	 * <p>
	 * @param cs cs
	 * @param sql sql
	 * @param args args
